...
function runMocha(browser) {
	global.uuu = 123333
	let mergedEnv = Object.assign(
		{ BROWSER: browser, TUNNEL: tunnel },
		process.env
	);
	require('fs').writeFileSync(mochaArgs, 'sss')
	console.log(mochaArgs, '!!!!!')
	let mocha = spawn("npm run mocha " + mochaArgs, { env: mergedEnv });
